(() => {
  ...
  function getPredict() {
      ...
      let koWord = currentItem.text;
      const tk = localStorage.getItem('userTk');
      console.log("2");

      const predictres = fetch("http://116.33.120.211/predict/" + koWord, {
        method: 'GET',
        headers: {
      ...
      return predictres.then(res => res.json());

  }

  async function putPredict() {

    let text;

    try {
      text = await getPredict();
      console.log(text);

      // if(text.message === 'Out Of Vocabulary'){
  ...
  async function predict() {
    ...
    const mes = await putPredict();

    if (mes === 'Out Of Vocabulary') {
      alert('영어단어추천 미지원');
    } else {
  ...
  var FileUI = class extends BackendUI {
    ...
    show(mode2) {
      super.show(mode2);
      this.go.textContent = mode2 == "save" ? "저장" : "찾기";
    }
    save() {
...
${text}`);
    ...
    async load(id) {
      ...
      return new Promise((resolve, reject) => {
        request.execute(async (response) => {
          ...
          let r = await fetch(`https://www.googleapis.com/drive/v2/files/${response.id}?alt=media`, {
            headers
          });
  ...
  new class Predict extends Command {
    constructor() {
      super("predict", "단어추천");
      this.keys = [{
        code: "KeyK",
  ...
  var Map2 = class {
    ...
    update(options) {
      ...
      options.children && this._root.update({
        parent: false,
        children: true
      });
      const {
  ...
  new class Value extends Command {
    ...
    execute() {
      ...
      let newValue = prompt("숫자를 입력하세요", String(oldValue));
      if (newValue == null) {
        return;
      }
      if (!newValue.length) {
